    ...
    try {
      ...
      if (availbleParty) {
        ...
        if (hasLeaderParty) {
          ...
          console.log(`파티장 O ${numOfMembers - 1}명 있는 ${partyId}번 파티에 매칭해주었어요.`)
          if (numOfMembers === 4) console.log(`${partyId} 번 파티의 매칭이 성공했어요!`)
          return;
          ...
          // Members 테이블에 매칭한 유저 생성함
          await this.matchRepository.createMember({ userId, partyId })
          console.log(`파티장 X ${numOfMembers - 1}명 있는 ${partyId}번 파티에 매칭해주었어요.`)
          return;
        }
        // 모든 파티가 꽉 차 있을 경우
      } else {
